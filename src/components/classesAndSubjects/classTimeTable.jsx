...
const ClassTimeTable = () => {
    ...
    if (currentView === "view" && timetable) {
      ...
      if (entry) {
        ...
        content = (
          <div className={`p-2 rounded ${subject?.color || "bg-gray-100"}`}>
            ...
            {/* CHANGE 1: Display full teacher name */}
            <div className="text-xs opacity-75 truncate" title={teacher?.name}>
              {teacher?.name}
            </div>
          </div>
        );
      ...
      if (cell && cell.subjectId) {
        ...
        content = (
          ...
          >
            <div className="font-semibold text-xs">{subject?.code}</div>
            {/* CHANGE 2: Display full teacher name */}
            <div className="text-xs opacity-75 truncate" title={teacher?.name}>
              {teacher?.name}
            </div>
          </div>
        ...
        content = (
          <div
            className="p-2 h-full min-h-[50px] border-2 border-dashed border-gray-300 rounded cursor-pointer hover:border-gray-400 flex items-center justify-center"
            onClick={() => setEditingCell({ dayIndex, slotIndex })}
          >
  ...
  const saveUpdatedCell = (updatedCell) => {
    setEditableGrid((prev) => {
      const newGrid = [...prev];
      newGrid[updatedCell.dayIndex][updatedCell.slotIndex] = {
        subjectId: updatedCell.subjectId || null,
        teacherId: updatedCell.teacherId || null,
      };
      return newGrid;
    });
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Create, view, and update class timetables
          </p>
        </div>
        ...
        {currentView === "select" && (
          <div className="bg-white shadow rounded-lg p-12 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Select a class and session to get started
            </p>
            <p className="mt-4 text-sm text-gray-600">
              Demo timetables available for 11th and 12th grade classes
            </p>
          </div>
        )}
          ...
          <div className="bg-white shadow rounded-lg p-12 text-center">
            ...
            <div className="mt-4 p-4 bg-blue-50 rounded-md">
              <p className="text-sm text-blue-800">
                Use the quick templates above to auto-fill a balanced timetable
                for Science or Commerce streams
              </p>
            </div>
          </div>
        )}
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-semibold text-gray-900">
                {classes.find((c) => c.id === selectedClass)?.name} - Section{" "}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {timeSlots.map((slot, slotIndex) => (
                    <tr
                      key={slot.id}
                      className={
                        slot.period === "Lunch Break" ? "bg-gray-50" : ""
                      }
                    >
        ...
        {editingCell && (
          <div className="fixed inset-0 z-20 flex items-center justify-center">
            ...
            {/* MODAL */}
            <div className="relative bg-gradient-to-br from-white to-gray-50 shadow-2xl rounded-2xl p-8 w-full max-w-md z-30 border border-gray-100">
              ...
              {/* SELECT SUBJECT */}
              <label className="block mb-2 text-sm font-semibold text-gray-600 uppercase tracking-wider">
                Select Subject
              </label>
              <select
                className="w-full p-3 border border-gray-300 rounded-lg mb-6 bg-white text-gray-700 shadow-sm transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button
                  className="px-6 py-2.5 bg-gray-100 text-gray-700 font-medium rounded-lg hover:bg-gray-200 transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
                  onClick={() => setEditingCell(null)}
                >
                  Cancel
                </button>
                ...
                <button
                  className="px-6 py-2.5 bg-blue-600 text-white font-medium rounded-lg shadow-sm hover:bg-blue-700 hover:shadow-md transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                  onClick={() => {
                    saveUpdatedCell(editingCell);
                    setEditingCell(null);
                  }}
                >
                  Save
